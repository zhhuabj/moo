...
def deploy(cfg, release, config, name, network, network_name, skip_network):
    ...
    if release == 'xenial':
        ...
        project_net_id = openstack.GetNetID(cfg.project_net)
        instance_nics = [{'net-id': project_net_id}, {'net-id': maasnet_id}]
...
@cli.command()
@pass_config
def configure_maas(cfg):
    """configure maas"""
    # https://git.launchpad.net/~canonical-support-eng/+git/stsstack/tree/infrastructure/maas/install.sh
    configuremaas = ConfigureMAAS(cfg)
    configuremaas.run('xenial', '10.12.1.2')
...
@click.option('--tag', default='maasstack', help='tag name for the node. default=maasstack')
@pass_config
def register_openstack_vm(cfg, name, image, flavor, tag):
    """register openstack vm into maas by NovaPowerDriver"""
    openstack = OpenstackUtils(cfg)
    if not openstack.Init():
        return
    instance_id = openstack.GetInstanceID(name)
